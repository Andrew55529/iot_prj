...
void SP_handleConnect() {
  ...
  strcpy(user_tmp, _SP_server.arg("user").c_str());
  String user_orig=portalCfg.user;
  String user_new=user_tmp;
  if (user_orig!=user_new) {
    change_user=true;
     strcpy(portalCfg.user, _SP_server.arg("user").c_str());
  }
